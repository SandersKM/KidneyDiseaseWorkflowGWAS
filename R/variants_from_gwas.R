...
for(n in 1:length(ld.eqtl.gwas.overlap.position$position)){
  zoom.ld.eqtl.gwas.overlap <- ld.eqtl.overlap[which(ld.eqtl.overlap$position.variation1 <
                                                  ...
                                                  ld.eqtl.overlap$position.variation1 >
                                                    ld.eqtl.gwas.overlap.position$position[n] - range.around.gwas &
                                                  ld.eqtl.overlap$position.variation2 <
                                                    ld.eqtl.gwas.overlap.position$position[n] + range.around.gwas &
                                                  ld.eqtl.overlap$position.variation2 >
                                                    ld.eqtl.gwas.overlap.position$position[n] - range.around.gwas),]

  zoom.ld.eqtl.gwas.overlap <- zoom.ld.eqtl.gwas.overlap[(zoom.ld.eqtl.gwas.overlap$d_prime > dprime.min |
                                                            zoom.ld.eqtl.gwas.overlap$variation1 %in%
                                                            ld.eqtl.gwas.overlap.position$rsid |
                                                            zoom.ld.eqtl.gwas.overlap$variation2 %in%
                                                            ld.eqtl.gwas.overlap.position$rsid),]

  # number of LD - eQTL overlaps in zoom
  zoom.ld.eqtl.gwas.overlap.row.num <- length(unique(union(zoom.ld.eqtl.gwas.overlap$variation1,
                                                           zoom.ld.eqtl.gwas.overlap$variation2)))
  ...
  zoom.ld.eqtl.gwas.overlap.rsids <- zoom.ld.eqtl.gwas.overlap.rsids[order(LD.info.500Kb.unique.variants$position[
    match(zoom.ld.eqtl.gwas.overlap.rsids,LD.info.500Kb.unique.variants$rsid)])]

  # BP postions in LD - eQTL overlap sorted smallest to largest
  zoom.ld.eqtl.gwas.overlap.positions <- LD.info.500Kb.unique.variants$position[
    match(zoom.ld.eqtl.gwas.overlap.rsids,LD.info.500Kb.unique.variants$rsid)]
  ...
  for(i in 1:dim(zoom.ld.eqtl.gwas.overlap)[1]){
    zoom.ld.eqtl.gwas.overlap.r2[zoom.ld.eqtl.gwas.overlap$variation1[i], zoom.ld.eqtl.gwas.overlap$variation2[i]]<-
      zoom.ld.eqtl.gwas.overlap.r2[zoom.ld.eqtl.gwas.overlap$variation2[i],zoom.ld.eqtl.gwas.overlap$variation1[i]] <-
  ...
  jpeg(file = paste(output.dir, gene.of.interest,"-GWAS-", ld.eqtl.gwas.overlap.position$rsid[n],"-LD_eQTL", ".jpeg", sep = ""),
